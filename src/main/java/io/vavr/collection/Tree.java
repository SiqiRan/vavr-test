...
public interface Tree<T> extends Traversable<T>, Serializable {
     ...
     * {@link java.util.stream.Stream#collect(Collector)} to obtain a {@link Tree}.
     *
    ...
    static <T> Tree<T> fill(int n, T element) {
        return Collections.fillObject(n, element, empty(), Tree::of);
    }
     ...
     * @throws UnsupportedOperationException if this tree is empty
     */
    T getValue();
...
interface TreeModule {
    ...
    static <T> Stream<Node<T>> traverseLevelOrder(Node<T> node) {
        ...
        final java.util.Queue<Node<T>> queue = new LinkedList<>();
        queue.add(node);
        while (!queue.isEmpty()) {
